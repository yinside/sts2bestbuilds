import { getPostBySlug, getAllPosts, getRelatedPosts, estimateReadingTime, extractHeadings } from '@/lib/posts';
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: post.title, description: post.description, type: 'article', tags: post.tags },
  };
}

const TYPE_BACKLINKS: Record<string, string> = {
  build: '/builds',
  guide: '/guides',
  boss: '/bosses',
  relic: '/relics',
...
export default async function PostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const backHref = TYPE_BACKLINKS[post.type] || '/';
  const backLabel = TYPE_LABELS[post.type] || 'Home';
      ...
      <article>
        <div className="bg-[var(--bg-elevated)] border-b border-[var(--border)]">
          <div className="container-site py-10 md:py-14">
            <Link href={backHref} className="inline-flex items-center gap-1 text-sm text-[var(--text-muted)] hover:text-white transition-colors mb-6">
              ...
              {related.length > 0 && (
                <div className="mt-12 pt-8 border-t border-[var(--border)]">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {related.map((r) => (
                      <Link key={r.slug} href={`/${r.slug}`} className="card-premium p-4 group">
